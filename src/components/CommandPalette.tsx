...
export default function CommandPalette({ open, onClose, onOpenSettings }: Props) {
  ...
  const commands: PaletteItem[] = useMemo(() => [
    ...
    { id: 'show-saas-ops', label: 'View: Show SaaS Launch Ops', category: 'command' as const, icon: <Rocket size={14} />, action: () => { dispatch('orion:show-saas-ops'); onClose() }, group: 'View' },
    { id: 'show-billing', label: 'SaaS: Open Billing and Entitlements', category: 'command' as const, icon: <CreditCard size={14} />, action: () => { dispatch('orion:open-account'); onClose() }, group: 'SaaS' },
